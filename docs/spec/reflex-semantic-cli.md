# REFLEX v0.2: Semantic decisions for Unix and CI

## One-line outcome

A developer can pipe text or JSON into `reflex` and receive a typed, confidence-aware semantic predicate, choice, score, or filtered stream that is safe to compose in shell scripts and CI.

## Why this replaces the original product thesis

The original action-firewall prototype is technically valid but is not the strongest standalone product. Cordum and several smaller projects already position themselves as agent action firewalls with approval workflows, policy engines, dashboards, and audit logs. Rebuilding that control plane would put REFLEX in a crowded, operationally heavy category where Jev is only one internal component.

Jev's differentiated capability is broader: cheap, fast, typed judgments (`Noul`, `Choice`, and `Score`) over arbitrary state, with many isolated questions evaluated in parallel. REFLEX should expose that capability as a small composable developer primitive. The existing agent guard remains as a built-in recipe and proof of composition rather than the entire product.

Positioning: **REFLEX is `jq` for judgment.**
...
- Broad evaluation is not the wedge. Promptfoo and DeepEval already cover full LLM evaluation suites. REFLEX focuses on one-shot semantic decisions inside ordinary developer workflows.
...
| Success condition | In under five minutes, a new user installs REFLEX and uses stdin or a file to run a predicate, route among choices, score a rubric, and filter JSONL; machine output and exit codes compose correctly in CI | verified | Directly testable public-interface target |
...
| Product identity | Keep the `REFLEX` name. Reposition from “agent firewall” to “semantic decisions for Unix and CI.” Keep `reflex guard` as an optional recipe | assumed | Name is already implemented and fits fast System One decisions; reversible |
...
printf '%s' 'Login from a new country, then payout details changed.' \
  | reflex is 'This describes a plausible account takeover'
```
...
The old `reflex check` command remains as a deprecated alias for one minor release.
...
|---|---|
| Installability | Build wheel and sdist; install each into separate clean virtual environments; `reflex --version` and all command help pages return 0 |
| Predicate true | Pipe an account-takeover narrative; `reflex is ... --json` returns `verdict=true` and exit 0 |
...
| Compatibility | Existing `reflex check` behavior remains correct and emits a deprecation warning to stderr |
...
6. **Question quality matters.** REFLEX cannot make vague or compound criteria reliable. Documentation and validation should encourage one atomic judgment per invocation.
7. **Package and command naming may collide.** Verify PyPI/GitHub availability before publication. Local implementation can proceed without reserving either.
